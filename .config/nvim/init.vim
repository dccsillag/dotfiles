...
for filetype in ['tex', 'markdown']
    ...
    call lexima#add_rule({ 'filetype': filetype, 'char': '<Bar>',   'at': '\\left\%#',                     'input_after': '\right|' })
...
call lexima#add_rule({ 'filetype': 'cpp',     'char': '<', 'at': '\([A-Za-z0-9_]\|^\s*template \|^#\s*include\s\+\)\%#', 'input_after': '>' })
