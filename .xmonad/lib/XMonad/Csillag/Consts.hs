...
csillagGridSelectConfig :: GSConfig a
csillagGridSelectConfig = GSConfig { gs_cellheight   = round size
    ...
    where
        ...
        colorizer _ active = withWindowSet $ \ws -> do
            return $ if active then (uiPrimaryBgColor, uiPrimaryFgColor)
                               else ( uiNormalBgColor,  uiNormalFgColor)
        def' = buildDefaultGSConfig colorizer

csillagWorkspaceGridSelectConfig :: GSConfig WorkspaceId
csillagWorkspaceGridSelectConfig = csillagGridSelectConfig { gs_colorizer = colorizer }
    where
